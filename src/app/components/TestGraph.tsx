...
// Helper to display dates in a readable format
...
// Used for placeholder demo when no repo is loaded
...
// Applies Dagre layout to nodes/edges
const getLayoutedElements = (nodes: Node[], edges: Edge[]) => {
  const dagreGraph = new dagre.graphlib.Graph();
  dagreGraph.setDefaultEdgeLabel(() => ({}));
  dagreGraph.setGraph({ rankdir: 'LR', ranksep: 100, nodesep: 80, marginx: 50, marginy: 50 });
...
function GraphInner({ repo }: TestGraphProps) {
  ...
  // Handle node click: open GitHub file or show contributor details
  ...
  // Fetch graph data on repo or graph mode change
  useEffect(() => {
    let cancelled = false;
  ...
  // Auto-fit the graph once layout is ready
  useEffect(() => {
  ...
  // Export current graph view as a JSON file
  const exportToJSON = () => {
  ...
  // Optional highlight styling for search matches
